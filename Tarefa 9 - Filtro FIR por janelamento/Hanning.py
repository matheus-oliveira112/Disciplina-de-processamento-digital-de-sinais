...
def filtro_hanning(signal_in, fs, fc, M):

    # --- filtro FIR com frequência de corte absoluta e janela Hanning ---
    h = signal.firwin(M + 1, cutoff=fc, fs=fs, window='hann')
...
y1_comruido = filtro_hanning(sinal_ruido, 512, 120, 32)
...
#ORDEM 48
inicio = time.perf_counter()
y2_comruido = filtro_hanning(sinal_ruido, 512, 120, 48)
fim = time.perf_counter()
print(f"Tempo de execução: {fim - inicio:.6f} segundos")
...
#ORDEM 64
inicio = time.perf_counter()
y3_comruido = filtro_hanning(sinal_ruido, 512, 120, 64)
fim = time.perf_counter()
print(f"Tempo de execução: {fim - inicio:.6f} segundos")
